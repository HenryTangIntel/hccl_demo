...
| Collective / Algorithm                             | Sketch of data movement                                                                                                                      | Traffic per rank                                                                                       | Scaling-factor $f(n)$                           |
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ----------------------------------------------- |
| **Ring** (broadcast / reduce-scatter / all-gather) | Each tensor is divided into $n$ equal chunks. Every chunk makes $n-1$ hops around the ring; each rank forwards what it has already received. | $(n-1)\times\dfrac{S}{n}$ bytes sent **and** $(n-1)\times\dfrac{S}{n}$ bytes received → total $(n-1)S$ | $\dfrac{T}{S}=\dfrac{(n-1)S}{S}=\dfrac{n-1}{n}$ |
| **All-reduce** (ring, two-phase)                   | A **reduce-scatter** ring followed by an **all-gather** ring.                                                                                | Each phase costs $(n-1)S$ traffic → two phases cost $2\,(n-1)S$.                                       | $f(n)=\dfrac{2\,(n-1)S}{S}=2\,\dfrac{n-1}{n}$   |
| **Tree reduce / tree broadcast**                   | $n-1$ sends up (or down) a k-ary tree; every send is only $S/n$.                                                                             | $(n-1)\times\dfrac{S}{n}$                                                                              | $f(n)=\dfrac{n-1}{n}$                           |
| **Pairwise all-to-all**                            | Each rank sends its full tensor to every other rank in $n-1$ parallel steps.                                                                 | $(n-1)S$ (sent) + $(n-1)S$ (received) → total $(n-1)S$ per direction                                   | $f(n)=n-1$                                      |
| **Point-to-point (`send/recv`)**                   | Single sender, single receiver.                                                                                                              | $S$                                                                                                    | $f(n)=1$                                        |
